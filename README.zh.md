...
- **思考模式** — 支持 DeepSeek V4/V3.2、OpenAI o 系列、Anthropic Claude 的扩展推理
- **图片理解** — 拖拽或粘贴图片（Ctrl+V）；聊天记录中以缩略图展示，点击可放大查看；默认使用 Qwen-VL 视觉模型
- **Agent 循环** — 多轮工具调用，可配置最大轮次，滑动窗口上下文管理
- **指令白名单** — 将常用 shell 命令加入白名单，跳过每次确认弹窗
- **上下文压缩** — 超过 80k token 自动压缩（DeepSeek V4 百万上下文模型阈值为 800k），支持 `/compact` 手动压缩
...
- **用户气泡折叠** — 长输入自动折叠，支持展开/收起切换
- **平滑聊天导航** — ↑↓ 按钮使用 easeInOutCubic 动画平滑滚动
